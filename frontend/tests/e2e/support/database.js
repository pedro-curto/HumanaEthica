...
const ACTIVITY_COLUMNS = "activity (id, application_deadline, creation_date, description, ending_date, name, participants_number_limit, region, starting_date, state, institution_id)";
const ENROLLMENT_COLUMS = "enrollment (id, enrollment_date_time, motivation, activity_id, volunteer_id)"
const PARTICIPATION_COLUMNS = "participation (id, acceptance_date, rating, activity_id, volunteer_id)";
...
Cypress.Commands.add('createParticipations', () => {
  ...
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ENROLLMENT_COLUMS + generateEnrollmentTuple(1, "Has vacancies and do not participate"	,1 ,3),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ENROLLMENT_COLUMS + generateEnrollmentTuple(2, "Has vacancies and participate"	,1 ,4),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ENROLLMENT_COLUMS + generateEnrollmentTuple(3, "Has no vacancies and participate"	,2 ,3),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
    query: "INSERT INTO " + ENROLLMENT_COLUMS + generateEnrollmentTuple(4, "Has no vacancies and do not participate	"	,2 ,5),
    credentials: credentials,
  })
  cy.task('queryDatabase',  {
